...
class MemberRelations {
  ...
  factory MemberRelations.fromJson(Map<String, dynamic> json) { final fid = json['fatherId']; final mid = json['motherId']; return MemberRelations(fatherId: (fid is String && fid.isNotEmpty) ? fid : null, motherId: (mid is String && mid.isNotEmpty) ? mid : null, spouseIds: List<String>.from(json['spouseIds'] ?? const []), childrenIds: List<String>.from(json['childrenIds'] ?? const []), customLinks: (json['customLinks'] as List<dynamic>? ?? []).map((e) => CustomLink.fromJson(Map<String, dynamic>.from(e))).toList()); }
...
class FamilyMember {
  ...
  factory FamilyMember.fromJson(Map<String, dynamic> json) => FamilyMember(id: json['id'], familyId: json['familyId'], schemaVersion: json['schemaVersion'] ?? 1, data: Map<String, dynamic>.from(json['data'] ?? {}), photoMediaId: json['photoMediaId'] ?? json['photoBase64'], relations: MemberRelations.fromJson(Map<String, dynamic>.from(json['relations'] ?? {})), createdBy: json['createdBy'] ?? '', lastEditedBy: json['lastEditedBy'] ?? '', createdAt: _parseDateTime(json['createdAt']), updatedAt: _parseDateTime(json['updatedAt']));
...
DateTime _parseDateTime(dynamic value) {
  ...
  if (value is num) {
    // Jackson may send epoch seconds (with decimal) or epoch milliseconds
    if (value < 1e12) {
